...
import ExperienceEducation from './dashboard/ExperienceEducation';
import ATSSummaryCard from './dashboard/ATSSummaryCard';
import ATSImprovementGuide from './dashboard/ATSImprovementGuide';
...
const Dashboard: React.FC<DashboardProps> = ({ result, onReset, onViewATS }) => {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-8">
          ...
          {result.atsScore && (
            <div>
              <ATSSummaryCard atsScore={result.atsScore} />
              <button
                onClick={onViewATS}
                className="w-full mt-4 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors"
              >
                📊 View ATS Analysis
              </button>
            </div>
      ...
      {/* ATS Improvement Guide (if score is low) */}
      {result.atsScore && result.atsScore.overallScore < 70 && (
        <div>
          <ATSImprovementGuide atsScore={result.atsScore} />
        </div>
      )}

      {/* Experience & Education Chronology */}
      <ExperienceEducation experience={result.experience} education={result.education} />
      ...
      <ReportGenerator ref={reportGeneratorRef} result={result} atsScore={result.atsScore?.overallScore} />
    </div>
  );
};
